...
#Config: {
	...
	// App settings.
	message!:        string
	nextHopHostname: *"" | string
	useIngress:      *false | bool
}
...
#Instance: {
	...
	objects: {
		...
		cme: #ConfigMapEnvs & {#config: config}
		if config.useIngress {ing: #Ingress & {#config: config}}

		deploy: #Deployment & {
			#config:  config
			#cmName:  objects.cm.metadata.name
			#cmeName: objects.cme.metadata.name
		}
	}
